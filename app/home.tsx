...
export default function Home() {
  ...
  const addToPlayList = async (songID: string) => {
    try {
      const response = await fetch(
        `${ngrok_url}/addToPlaylist?playlist_id=${playlistId}&song_id=${songID}`,
        {
          method: "GET",
          headers: {
  ...
  const getNextSong = async () => {
    try {
      ...
      const response = await fetch(
        `${ngrok_url}/getNextSong?playlist_id=${playlistId}`,
        {
          method: "GET",
          headers: {
            "Content-Type": "application/json",
  ...
  return (
    <View style={styles.container}>
      ...
      >
        <Image source={{ uri: songData.image }} style={styles.albumArt} />
      </Animated.View>
